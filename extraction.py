...
Key components:
    ...
    * Synonym handling and regex-extraction for non-benefits realted data
    * Incentive extraction, industry category detection and experience knwoledge extraction using LLMs
    * Classification using Sentence-Transformer with few-shot learning and context informations
...
def get_direct_benefits(job):
    ...
    Adds them to the :func:`classification<extraction.process_jobs>` together with the incentives found by the LLM.
    """
...
def classify_incentives_with_few_shot(incentives, device, threshold=0.45):
    ...
    This function uses a SentenceTransformer model to compare job incentives against
    ...
    It further creates a looging-file which shows the found incentives with their 
...
"""
with open("/home/lukas/Dokumente/countries+states+cities.json") as f:
...
def detect_homeoffice(job_data):
    ...
    2. Fall back to full-text search across all job description sections
    3. Return 1 if any homeoffice pattern is found, 0 otherwise
    ...
    This direct pattern matching approach is more efficient than using LLM-based
...
def sanitize_json_response(response):
    ...
    sequences (\x00-\xFF) that are not valid in standard json and could cause
    parsing errors.
...
def parse_json_incentives(response):
    ...
    a list (potentially empty) without raising exceptions to the caller.
...
def clean_incentives(response):
    """
    Extract bulleted list items from model response when json parsing fails.
...
def process_jobs(insert_or_replace_job, batch_size=1):
    ...
    1. Extracts job data from Mongo-DB
    2. Processes job descriptions using Llama 3.2 for information extraction
    3. Classifies incentives/benefits using sentence transformers
    4. Stores the processed data in a SQLite-Database
    
    Key design choices (s. also :ref:`here<Model Selection and Implementation>`):

    - Models are loaded and unloaded for each batch to manage memory efficiently
    - Different prompts target required experiences, incentives/benefits and the branche
    - No Few-Shot Learning for the LLMs due to halucination of examples
    - Temperature settings vary by task (0.1 for factual extraction, higher for creative tasks)
    - Repetition penalties (1.1-1.2) prevent the model from repeating itself
    - The LLM for experience requirements receives the full job description
    - The LLM for incentive extraction only receives benefits-related text to reduce noise
    - Sentence transformers are used for classification to improve efficiency with a general threshold of 0.45
